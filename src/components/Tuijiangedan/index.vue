...
import {getTuijianData,getSongUrlByMid} from 'api/api.js'
...
export default {
    ...
    methods:{
      ...
      handleData(data){
        ...
        result =data.map((item,index)=>{
          let {albummid,albumname,songname,singer,songmid}=item
    ...
    async created(){
      ...
      let res= await getTuijianData(disstid)
        console.log(res)
        let{result,mids} = this.handleData(res.cdlist[0].songlist)
        ...
        this.name=res.cdlist[0].dissname
        this.bg=res.cdlist[0].logo
            this.$nextTick(()=>{
              this.initBs()
            })
    }
